...
import ThemeCard from "../components/ThemeCard.vue"; // Import ThemeCard component
...
export default {
    ...
    data() {
        return {
            themes: {}, // ใช้สำหรับเก็บข้อมูล Pagination
            countries: {
                "": "ทั้งหมด",
                th: "ไทย",
                jp: "ญี่ปุ่น",
    ...
    computed: {
        headerTitle() {
            if (this.category === "official") {
                return "ธีมไลน์ทางการ";
            } else if (this.category === "creator") {
                return "ธีมไลน์ครีเอเตอร์";
            } else {
                return "ธีมไลน์ทั้งหมด";
            }
        },
    ...
    watch: {
        ...
        "$route.query": {
            ...
            handler(newQuery) {
                ...
                this.fetchThemes(page, this.category, country, order);
            },
        },
    },
    methods: {
        async fetchThemes(page, category, country, order) {
            try {
                // สร้าง URL จากพารามิเตอร์
                const url = `/api/theme-more?page=${page}&category=${category}&country=${country}&order=${order}`;
                const response = await axios.get(url);
                this.themes = response.data; // ดึงข้อมูล Pagination จาก API
